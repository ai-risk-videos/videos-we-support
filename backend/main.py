...
FORMAT_RULE = ("" if IDEA_FORMAT == "title" else
    ...
    "BUT VARY IT across the whole list: the 'what happens when...?' question is ONE tool, not the default. Do NOT "
    "end most ideas on a question. The MAJORITY of closers should be flat declarative statements of the concrete "
    "stake (equally clear, e.g. 'Nobody voted for that.' / 'The control problem stays real and unsolved.'); use the "
    ...
    "READING LEVEL: write the summary so a smart 10-year-old (about 5th grade) gets it on the FIRST read. Short "
    "common words, short sentences. The biggest thing that makes these too hard is ABSTRACT NOUNS stacked together "
    "(nominalizations): 'the gap between the order and the intent', 'the concentration of power', 'the erosion of "
...
(g) READING LEVEL: write so a smart 10-year-old (about 5th grade) gets it on the FIRST read. Short common words, short sentences. The #1 thing that makes these too hard is ABSTRACT NOUNS stacked together (nominalizations): 'the gap between the order and the intent', 'the concentration of power', 'the erosion of human oversight', 'a loss of alignment'. Turn every abstract noun back into PEOPLE or THINGS doing something plain. Corrections: BAD (too abstract) 'That gap between the order and the intent behind it stays open, and no one has closed it yet.' GOOD (5th grade) 'They can tell an AI to chase a goal, but they still cannot say exactly what they want. Nobody has fixed that.' BAD 'The concentration of power accelerates.' GOOD 'A few companies get more powerful, and faster.' BAD 'This risks the erosion of meaningful human oversight.' GOOD 'Soon no person is really checking what the AI does.' Keep every real fact and name; just say it in small, plain words.
