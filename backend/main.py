...
async def similar(req: Request):
    ...
    try:
        ...
        # WHEN, AND BY WHICH VERSION OF THE WRITER. Stamped per idea rather than per page, because
        # a page accumulates ideas over several sessions ("+ get more") and the whole point is to
        # be able to bin the stale ones without touching the good ones beside them.
...
# version." _DEPLOY_STAMP cannot do that job: it is the server's start time, so a Railway restart,
# a crash loop or a sleep/wake bumps it while the writing is byte-identical. Filtering on it would
# condemn ideas that are actually current.
#
...
def _gen_version():
    ...
    for name in ("SYSTEM_CUSTOM", "ONEBLOCK_FORMAT", "ANTI_SLOP", "READING_LEVEL", "TRAJECTORY",
                 "BOLD_ENDGAME_SYS", "SENTENCE_FIX_SYS", "GRADE_FIX_SYS", "ENDGAME_GRADE_SYS",
                 "FIDELITY_FIX_SYS", "ACTIVATE_SYS"):
        parts.append(str(globals().get(name, "")))
    h = hashlib.sha256("\x00".join(parts).encode("utf-8", "ignore")).hexdigest()[:8]
    return "g" + h
