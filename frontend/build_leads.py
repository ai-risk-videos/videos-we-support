...
/* DRAG TO REORDER. The arrows move one slot at a time; the curator often needs to move an idea 5 to 15
   places, which is 15 clicks and a re-render each. Native HTML5 drag and drop, no library. */
.ccard[draggable="true"]{cursor:grab}
.ccard.dragging{opacity:.4;cursor:grabbing}
.ccard.dropafter{box-shadow:0 3px 0 0 #6fb3ff}
.ccard.dropbefore{box-shadow:0 -3px 0 0 #6fb3ff}
.cgrip{position:absolute;top:9px;right:11px;color:#5a6673;font-size:14px;letter-spacing:1px;cursor:grab;user-select:none}
.cgrip:hover{color:var(--ink)}
/* IDEA STATUS. Four states, so we can see ready-vs-generated at a glance, stop re-reviewing settled
   ideas, and send creators a link with only approved ideas on it. Rejection stays the existing
...
.stbtn.on{font-weight:650}
.stbtn.on[data-st="new"]{background:#2a2f36;color:#dbe3ea;border-color:#39424c}
.stbtn.on[data-st="pre"]{background:#3a2c12;color:#e8bd6a;border-color:#6b5220}
.stbtn.on[data-st="ok"]{background:#16301f;color:#78d79b;border-color:#2c6741}
.stcount{display:flex;gap:7px;flex-wrap:wrap;align-items:center;margin:0 0 11px}
.stchip{font-size:12px;padding:5px 10px;border-radius:999px;border:1px solid var(--line);background:transparent;color:var(--mut);cursor:pointer}
.stchip b{color:var(--ink);margin-right:4px}
.stchip.sel{border-color:#6fb3ff;color:var(--ink)}
.stchip.ok b{color:#78d79b}.stchip.pre b{color:#e8bd6a}
...
.morebtn.working{border-color:#9ac47f;background:#1d2a18;cursor:pointer}
...
function startCurate(ideas,handle,channel,note,profile,style,rejected){
 ...
 list.innerHTML='<div class="curstep">Reviewing the ideas for <b>'+esc(curateChannel)+'</b>. Every change saves automatically.</div>'+
  ...
  '<div class="stcount" id="stcount"></div>'+
  '<div class="chansub" style="margin:-4px 0 10px">Drag a card by the ⠿ handle to move it any distance. The ▲▼ buttons still nudge one slot.</div>'+
  '<div id="ccards"></div>'+
  '<button class="morebtn" id="moreideas">＋ Get 50 more ideas (keeps the ones you have)</button>'+
  '<div id="rejwrap"></div>';
...
// Bulk "more ideas". The goal is 20-30 KEEPERS on a creator's page, so the curator needs to see far
// more than that and cut. One generation call takes ~200s and returns at most ~28, and asking a
...
async function addMoreIdeas(){
 ...
 if(btn){btn.disabled=false;btn.classList.add("working");}
 setLabel("Finding ideas… 0 of "+MORE_TARGET+"  (click to stop)");
 // while it runs the button becomes a stop control, so a 10 minute job is never a trap
 const stopper=()=>{moreCancel=true;setLabel("Stopping after this batch…");};
 if(btn){btn.onclick=stopper;}
 try{
  while(added<MORE_TARGET && round<MORE_MAX_ROUNDS && !moreCancel){
   round++;
   ...
   if(newThisRound===0){dry++;if(dry>=2)break;continue;}
   dry=0;added+=newThisRound;
   // paint after EVERY batch so the curator can start reading while the rest generate
   curateDirty=true;renderCards();scheduleSave();
   toast("Added "+newThisRound+" more ("+added+" this run)");
  }
 } finally {
  if(btn){btn.disabled=false;btn.classList.remove("working");btn.textContent=orig;btn.onclick=addMoreIdeas;}
 }
 if(added){
  const cards=document.querySelectorAll("#ccards .ccard");
  if(cards.length>=added)cards[cards.length-added].scrollIntoView({behavior:"smooth",block:"center"});
  toast("Done: "+added+" new ideas added. You now have "+curateIdeas.length+".");
 } else {
  toast("No genuinely new ideas came back. Try once more, or reject a few to steer it.");
 }
}

// ---- IDEA STATUS ------------------------------------------------------------------------------
// "" / "new"  Not reviewed   (fresh generation nobody has looked at)
// "pre"       Needs Drew's approval  (pre-approved by the first reviewer)
// "ok"        Approved  (ready to show creators)
...
function pageUsesStatus(ideas){return (ideas||[]).some(x=>x&&(x.st==="ok"||x.st==="pre"));}
function publishableIdeas(ideas){
 return pageUsesStatus(ideas)?(ideas||[]).filter(x=>ideaSt(x)==="ok"):(ideas||[]);
}
let stFilter="all";
function renderStatusBar(){
 const el=$("#stcount");if(!el)return;
 const c={"new":0,"pre":0,"ok":0};curateIdeas.forEach(x=>{c[ideaSt(x)]++;});
 const chip=(k,lbl,extra)=>'<button class="stchip '+(extra||"")+(stFilter===k?" sel":"")+'" data-f="'+k+'"><b>'+
   (k==="all"?curateIdeas.length:c[k])+'</b>'+lbl+'</button>';
 el.innerHTML=chip("all","total")+chip("new","not reviewed")+chip("pre","need Drew","pre")+chip("ok","approved","ok")+
  '<span class="chansub" style="margin-left:auto">'+(c.ok?("Publishing sends the "+c.ok+" approved"):"No approved ideas yet, publishing sends everything")+'</span>';
 el.querySelectorAll("[data-f]").forEach(b=>b.onclick=()=>{stFilter=b.getAttribute("data-f");renderCards();});
}
let _dragFrom=null;
function _clearDropHints(){document.querySelectorAll("#ccards .ccard").forEach(c=>c.classList.remove("dropafter","dropbefore"));}
function renderCards(){
 const w=$("#ccards");if(!w)return;
 const ct=$(".curtitle");if(ct)ct.textContent=curTitle();
 renderStatusBar();
 w.innerHTML=curateIdeas.map((x,i)=>{
  if(stFilter!=="all"&&ideaSt(x)!==stFilter)return "";
 ...
 w.querySelectorAll(".ccard").forEach(card=>{
  ...
  card.addEventListener("dragstart",ev=>{
   _dragFrom=i;card.classList.add("dragging");
   try{ev.dataTransfer.setData("text/plain",String(i));ev.dataTransfer.effectAllowed="move";}catch(e){}
  });
  card.addEventListener("dragend",()=>{card.classList.remove("dragging");_clearDropHints();_dragFrom=null;});
  card.addEventListener("dragover",ev=>{
   if(_dragFrom===null||_dragFrom===i)return;
   ev.preventDefault();ev.dataTransfer.dropEffect="move";
   const r=card.getBoundingClientRect();const after=(ev.clientY-r.top)>r.height/2;
   _clearDropHints();card.classList.add(after?"dropafter":"dropbefore");
  });
  card.addEventListener("dragleave",()=>card.classList.remove("dropafter","dropbefore"));
  card.addEventListener("drop",ev=>{
   ev.preventDefault();
   const from=_dragFrom;_clearDropHints();
   if(from===null||from===i)return;
   const r=card.getBoundingClientRect();const after=(ev.clientY-r.top)>r.height/2;
   let to=after?i+1:i;
   const a=curateIdeas;const moved=a.splice(from,1)[0];
   if(from<to)to--;                       // the splice above shifted everything after `from` down one
   a.splice(to,0,moved);
   _dragFrom=null;curateDirty=true;renderCards();scheduleSave();
   toast("Moved to position "+(to+1));
  });
  card.querySelectorAll(".stbtn").forEach(b=>b.onclick=()=>{
   const k=b.getAttribute("data-st");
   curateIdeas[i].st=(k==="new")?"":k;   // store "" for not-reviewed so old pages stay byte-identical
   curateDirty=true;renderCards();scheduleSave();
  });
  // an approved idea is settled; make destroying it deliberate rather than one stray click
  card.querySelector('[data-act="rm"]').onclick=()=>{
   if(ideaSt(curateIdeas[i])==="ok"&&!confirm("This idea is approved. Reject it anyway?"))return;
   curateDirty=true;const rm=curateIdeas.splice(i,1)[0];if(rm&&(ideaTitle(rm).trim()||ideaSummary(rm).trim()))curateRejected.unshift(rm);renderCards();renderRejected();scheduleSave();}; // removed ideas go to the reject pile (kept as signal), not discarded
...
async function publishCurrent(){
 ...
 // The doc keeps every idea; the creator link shows only the approved ones. Say which is which, and
 // refuse to publish a page whose creator view would be a surprise.
 const _vis=publishableIdeas(ideas);
 if(pageUsesStatus(ideas)&&!_vis.length){toast("Nothing is approved yet, so the creator would see nothing. Approve at least one idea first.");return;}
 if(pageUsesStatus(ideas)&&_vis.length<ideas.length&&
    !confirm("The creator will see the "+_vis.length+" approved idea"+(_vis.length===1?"":"s")+", not all "+ideas.length+". Publish?"))return;
 ...
 try{
  ...
  showPublished(location.origin+location.pathname+"?p="+encodeURIComponent(id),curateChannel,_vis.length);
...
async function showDashboard(){
 ...
 try{
  ...
  $("#dashbody").outerHTML=rows.map(r=>{
   ...
   const _n=(r.ideas&&r.ideas.length)||0, _ok=publishableIdeas(r.ideas||[]).length, _st=pageUsesStatus(r.ideas||[]);
   return '<div class="dashrow"><span class="dchan">'+esc(r.channel||r.handle||r.id)+'</span><span class="dmeta">'+
    (_st?(_ok+' approved of '+_n):(_n+' videos'))+' · '+when+'</span>'+
...
async function renderCreatorPage(id){
 ...
 // A creator link shows APPROVED ideas only, so a fresh generation sitting on the page waiting for
 // review never appears to them. The full set stays in the doc, which is what "Edit this page" and
 // the version history need. Pages made before statuses existed have none set, so they show
 // everything exactly as before; that fallback is the whole reason this is safe to ship.
 const _allIdeas=doc.ideas.slice();
 doc.ideas=publishableIdeas(doc.ideas);
 if(!doc.ideas.length)doc.ideas=_allIdeas; // never show a creator an empty page
 ...
 if(isAdmin()){
  ...
  ed.onclick=()=>{ed.remove();document.body.classList.remove("creator");creatorView=false;const pm2=document.querySelector(".premakebtn");if(pm2)pm2.remove();startCurate(_allIdeas,doc.handle||pageId(id),doc.channel||doc.handle||id,doc.note||"",doc.profile||"",doc.style||"leads",doc.rejected||[]);};
  ...
  pm.onclick=()=>premakeAll(pm,doc.ideas);  // packs only for what the creator can actually see
