...
async function _renderScreen(s){s=s||{v:"home"};
 ...
 if(s.v==="page"){await renderCreatorPage(s.id);}
  ...
  if(doc&&Array.isArray(doc.ideas)&&doc.ideas.length)startCurate(doc.ideas,doc.handle||s.h,doc.channel||doc.handle||s.h,doc.note||"",doc.profile||"",doc.style||"leads");
  else showHome();}
 else{showHome();}}
...
async function savePage(handle,channel,note,ideas,profile,style){
 ...
 try{
  await DB.runTransaction(async function(tx){
   ...
   tx.set(ref,Object.assign({},base,{history:history}));
  });
 }catch(e){
...
let curateIdeas=[], curateHandle="", curateChannel="", curateProfile="", curateStyle="leads", curateDirty=false, curateSaveT=null;
...
async function doAutoSave(){
 ...
 _setSaveInd("saving");
 try{await savePage(curateHandle,curateChannel,note,ideas,curateProfile,curateStyle);_setSaveInd("saved");}
...
function startCurate(ideas,handle,channel,note,profile,style){
...
async function regenerate(mode){
 ...
 let ok=false;
 try{ ok = (mode==="custom") ? await fetchCustom(url) : await fetchTailor(url,false); }catch(e){ ok=false; }
 _stopR();
...
function renderCards(){
 ...
 w.querySelectorAll(".ccard").forEach(card=>{
  ...
  card.querySelector('[data-act="rm"]').onclick=()=>{curateDirty=true;curateIdeas.splice(i,1);renderCards();scheduleSave();};
...
async function publishCurrent(){
 ...
 try{
  ...
  const id=await withTimeout(savePage(curateHandle,curateChannel,note,ideas,curateProfile,curateStyle),15000);
...
async function renderCreatorPage(id){
 ...
 if(isAdmin()){
  ...
  ed.onclick=()=>{ed.remove();document.body.classList.remove("creator");creatorView=false;const pm2=document.querySelector(".premakebtn");if(pm2)pm2.remove();startCurate(doc.ideas,doc.handle||pageId(id),doc.channel||doc.handle||id,doc.note||"",doc.profile||"",doc.style||"leads");};
...
async function fetchCustom(rawurl){
 const url=cleanChanUrl(rawurl);if(!url){toast("Paste a channel link");return false;}
 ...
 try{
  ...
  const r=await fetch(CUSTOM_API,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({channelUrl:url}),signal:ctrl.signal});
  clearTimeout(to);
  if(r.status===429){if(msg){msg.className="cmsg err";msg.textContent="Busy right now — wait a minute and try again.";}return false;}
  const j=await r.json();
